...
/** Just the number, rounded to one decimal in the browser's own locale - no unit. */
export function formatEmissionsNumber(gCO2e: number): string {
  return EMISSIONS_FORMATTER.format(gCO2e);
}

/**
 * Formats a gCO2e amount for display: always grams, rounded to one
 * decimal, decimal separator from the browser's own locale (not tied to the
 * fr/en interface language - a user in any locale sees their own
 * convention, e.g. "0,1 gCO2e" in French vs "0.1 gCO2e" in English).
 */
